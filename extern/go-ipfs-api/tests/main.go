...
func test2() {
	...
	out, err := sh.AddDir("/home/codex/test/my")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func test3(fileHash string) bool {

	sh = shell.NewShell("localhost:5001")
	lslinks, err := sh.List(fileHash)
...
func CheckIpfsAlive(url, fileHash string) bool {
	r := false

	cl := shell.NewShell(url)
...
func main() {
	...
	if !CheckIpfsAlive("http://127.0.0.1:5001", "QmTKZA24mw8gNTLnZeTZC6maSaHoDy8VB4eWBbtKF4gF1Z") {
		fmt.Println("CheckIpfsAlive return FALSE!")
		return
	}
	fmt.Println("CheckIpfsAlive return TRUE!")

	//test3("QmTKZA24mw8gNTLnZeTZC6maSaHoDy8VB4eWBbtKF4gF1Z")
